...
function doGet(e) {
  ...
  try {
    if (action === 'list') {
      ...
      // 以前はチャンクの一時保存先としてGoogleドキュメントを都度作成・開閉していたが、
      // ドキュメントの開く/保存する処理は1回あたり数秒かかり、チャンク数が多いファイルだと
      // アップロード全体で数分かかる原因になっていた。読み書きが高速なCacheServiceに
      // 切り替えて高速化する（この段階ではDriveへのアクセスも不要になった）。
      var tempId = Utilities.getUuid();
      CacheService.getScriptCache().put('upload_' + tempId + '_count', '0', 21600);
      out = { success: true, tempDocId: tempId };
    } else if (action === 'uploadChunkAppend') {
      var cacheA = CacheService.getScriptCache();
      var tempIdA = e.parameter.tempDocId;
      var countKeyA = 'upload_' + tempIdA + '_count';
      var countA = parseInt(cacheA.get(countKeyA) || '0', 10);
      cacheA.put('upload_' + tempIdA + '_' + countA, e.parameter.data || '', 21600);
      cacheA.put(countKeyA, String(countA + 1), 21600);
      out = { success: true };
    } else if (action === 'uploadChunkFinish') {
      var cacheF = CacheService.getScriptCache();
      var tempIdF = e.parameter.tempDocId;
      var countF = parseInt(cacheF.get('upload_' + tempIdF + '_count') || '0', 10);
      var keysF = [];
      for (var pi = 0; pi < countF; pi++) keysF.push('upload_' + tempIdF + '_' + pi);
      var chunkMapF = cacheF.getAll(keysF); // 1件ずつgetするより、まとめて取得したほうが速い
      var partsF = [];
      for (var pi2 = 0; pi2 < countF; pi2++) partsF.push(chunkMapF[keysF[pi2]] || '');
      var fullText = partsF.join('');
      ...
      cacheF.removeAll(keysF);
      cacheF.remove('upload_' + tempIdF + '_count');
